...
export function ProductListing({ onNavigate }: ProductListingProps) {
  ...
  const { activeGiftBoxes, loading: catalogLoading, error, fetchActiveGiftBoxes } =
    useCatalog();
  ...
  const giftBoxApi = {
    getActive: async () => ({
      data: {
        success: true,
        data: await fetchActiveGiftBoxes(),
      },
    }),
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setFetchError(null);
        const res = await giftBoxApi.getActive();
        if (res.data.success) {
