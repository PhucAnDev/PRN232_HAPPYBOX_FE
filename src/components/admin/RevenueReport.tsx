...
export function RevenueReport() {
  ...
  const { summary, salesTrend, orderStatus, loading, fetchSnapshot } =
    useDashboard();
  const summaryData: DashboardSummaryResponse | null = summary;
  const orderStatusData: OrderStatusChartDto[] = orderStatus;
  const salesTrendData: SalesTrendDto[] = salesTrend;

  const fetchData = async (start: string, end: string) => {
    try {
      await fetchSnapshot(start, end, 5);
    } catch (error) {
  ...
  const chartData = salesTrendData.map((day) => ({
