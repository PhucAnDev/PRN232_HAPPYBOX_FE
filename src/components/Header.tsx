...
export function Header({ cartCount, onNavigate, onCartClick, isLoggedIn = false }: HeaderProps) {
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            >
              <button 
                ...
                className={`text-gray-700 hover:text-[#B71C1C] font-medium transition-colors relative ${
                  showQuaTetDropdown ? 'text-[#B71C1C]' : ''
                }`}
              >
                Quà Tết
                {showQuaTetDropdown && (
                  <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-[#B71C1C]"></span>
                )}
              </button>
              
              {/* Dropdown Menu */}
              {showQuaTetDropdown && (
                <div className="absolute top-full left-0 pt-3 -ml-4">
